...
int Lighting_Colors()
{
	...
	Shader shader_cube("shaders/lighting_colors.vs", "shaders/lighting_colors.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::vec4 pos = view* glm::vec4(0, 0, 0, 1);

		glm::mat4 project = glm::perspective(glm::radians(45.f), ((float)SCR_WIDTH) / SCR_HEIGHT, 0.2f, 100.f);

		shader_light.use();
		glm::vec3 lightPos(1.f, 1.0f, 0.0f);
		...
		shader_light.setMat4("project", project);
		shader_light.setVec3("color", glm::vec3(1.f, 1.f, 1.f));
		...
		model = glm::mat4(1.0f);
		model = glm::rotate(model, 30.f, glm::vec3(0.f, 1.f, 0.f));
		shader_cube.use();
